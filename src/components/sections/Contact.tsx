import { useState } from 'react';
import { motion } from 'framer-motion';
import { Container } from '../layout/Container';
import profileImage from '../../assets/ID9.png';

/**
 * Contact Page Component
 * Matches adhamdannaway.com/contact design exactly
 */

interface ContactFormData {
  name: string;
  email: string;
  message: string;
}

const socialLinks = [
  { name: 'Twitter', href: 'https://twitter.com', color: '#1DA1F2' },
  { name: 'Facebook', href: 'https://facebook.com', color: '#4267B2' },
  { name: 'LinkedIn', href: 'https://linkedin.com', color: '#0077B5' },
  { name: 'Instagram', href: 'https://instagram.com', color: '#E4405F' },
];

export function Contact() {
  const [formData, setFormData] = useState<ContactFormData>({
    name: '',
    email: '',
    message: '',
  });

  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    
    // Simulate form submission
    await new Promise((resolve) => setTimeout(resolve, 1000));
    
    setFormData({ name: '', email: '', message: '' });
    setIsSubmitting(false);
  };

  return (
    <div className="bg-[#FFFFFF] min-h-screen">
      {/* TOP CONTACT HERO SECTION */}
      <section className="bg-[#FFFFFF] py-16 md:py-24 lg:py-32">
        <Container>
          <div className="grid md:grid-cols-2 items-center gap-10 lg:gap-16">
            {/* LEFT SIDE - Text Content */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, ease: 'easeOut' }}
            >
              {/* Large bold heading */}
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-[#333] tracking-tight">
                contact.
              </h1>
              
              {/* Paragraph below heading */}
              <p className="text-[#666] text-base md:text-lg mt-6 mb-10 max-w-md leading-relaxed">
                I'm currently available for freelance work and full-time positions. 
                Feel free to reach out if you have a project in mind or just want to say hello.
              </p>

              {/* Social Links - 2x2 Grid */}
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                {socialLinks.map((social, index) => (
                  <motion.a
                    key={social.name}
                    href={social.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.4, ease: 'easeOut', delay: 0.1 + index * 0.1 }}
                    className="flex items-center gap-3 group"
                  >
                    {/* Colored circular icon */}
                    <div 
                      className="w-10 h-10 rounded-full flex items-center justify-center transition-transform duration-200 group-hover:scale-110"
                      style={{ backgroundColor: social.color }}
                    >
                      {social.name === 'Twitter' && (
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="white">
                          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                        </svg>
                      )}
                      {social.name === 'Facebook' && (
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="white">
                          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg>
                      )}
                      {social.name === 'LinkedIn' && (
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="white">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                        </svg>
                      )}
                      {social.name === 'Instagram' && (
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="white">
                          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                        </svg>
                      )}
                    </div>
                    {/* Text label */}
                    <span className="text-[#333] text-sm font-medium group-hover:text-[#666] transition-colors">
                      {social.name}
                    </span>
                  </motion.a>
                ))}
              </div>
            </motion.div>

            {/* RIGHT SIDE - Large Portrait Image */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, ease: 'easeOut', delay: 0.2 }}
              className="relative"
            >
              {/* Large portrait image - slightly overflow bottom */}
              <div className="relative overflow-visible">
                <img
                  src={profileImage}
                  alt="Israfil Diganta"
                  className="w-full h-auto object-cover md:translate-y-4"
                />
              </div>
            </motion.div>
          </div>
        </Container>
      </section>

      {/* EMAIL FORM SECTION */}
      <section className="bg-[#FFFFFF] py-16 md:py-24">
        <Container>
          <div className="max-w-4xl mx-auto">
            {/* Heading */}
            <motion.h2
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, ease: 'easeOut' }}
              className="text-3xl md:text-4xl font-bold text-[#333] mb-10"
            >
              Send me an email
            </motion.h2>

            {/* Form */}
            <motion.form
              onSubmit={handleSubmit}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, ease: 'easeOut', delay: 0.1 }}
            >
              <div className="grid md:grid-cols-2 gap-8">
                {/* Left Column - Name and Email */}
                <div className="space-y-6">
                  {/* Name Input */}
                  <div>
                    <label 
                      htmlFor="name" 
                      className="block text-sm text-[#666] mb-2"
                    >
                      Name
                    </label>
                    <input
                      type="text"
                      id="name"
                      name="name"
                      value={formData.name}
                      onChange={handleChange}
                      required
                      className="w-full px-4 py-3 bg-white border border-[#ddd] rounded-md 
                               text-[#333] placeholder-[#999] 
                               focus:outline-none focus:border-[#333] focus:ring-1 focus:ring-[#333]
                               transition-colors duration-200"
                      placeholder="Your name"
                    />
                  </div>

                  {/* Email Input */}
                  <div>
                    <label 
                      htmlFor="email" 
                      className="block text-sm text-[#666] mb-2"
                    >
                      Email
                    </label>
                    <input
                      type="email"
                      id="email"
                      name="email"
                      value={formData.email}
                      onChange={handleChange}
                      required
                      className="w-full px-4 py-3 bg-white border border-[#ddd] rounded-md 
                               text-[#333] placeholder-[#999] 
                               focus:outline-none focus:border-[#333] focus:ring-1 focus:ring-[#333]
                               transition-colors duration-200"
                      placeholder="your@email.com"
                    />
                  </div>
                </div>

                {/* Right Column - Message */}
                <div>
                  <label 
                    htmlFor="message" 
                    className="block text-sm text-[#666] mb-2"
                  >
                    Message
                  </label>
                  <textarea
                    id="message"
                    name="message"
                    value={formData.message}
                    onChange={handleChange}
                    required
                    rows={5}
                    className="w-full px-4 py-3 bg-white border border-[#ddd] rounded-md 
                             text-[#333] placeholder-[#999] 
                             focus:outline-none focus:border-[#333] focus:ring-1 focus:ring-[#333]
                             transition-colors duration-200 resize-none h-full min-h-[140px]"
                    placeholder="Your message..."
                  />
                </div>
              </div>

              {/* Submit Button - Right aligned */}
              <div className="mt-8 flex justify-end">
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="px-8 py-3 bg-[#333] text-white text-base font-medium rounded-md
                           hover:bg-[#222] transition-colors duration-200
                           disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? 'Sending...' : 'Send Message'}
                </button>
              </div>
            </motion.form>
          </div>
        </Container>
      </section>


    </div>
  );
}

export default Contact;
